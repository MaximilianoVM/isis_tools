...
def format_archives(header_file, data_file): 
    # cargamos archivos
    header_info = pd.read_csv(header_file, delim_whitespace=True, names=['file', 'JD', 'HJD', 'UT', 'EXPTIME', 'FILTER'])

    lc_data = pd.read_csv(data_file, delim_whitespace=True, names=['HJD', 'Flujo', 'errFlujo', 'refFlujo', 'errRefF', 'calSNR'])
...
def merge_data(lc_data, header_info): 
    merged_df = pd.merge_asof(
        lc_data,                # DataFrame base
        header_info,            # DataFrame a unir
        on="HJD",               # columna clave
...
def instrumental_magnitude(merged_df): 
    ...
    merged_df['Flujo'] = 25.0 - 2.5*np.log10(merged_df['Flujo']) + 2.5*np.log10(merged_df['EXPTIME'])
    return merged_df
